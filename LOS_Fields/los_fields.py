...
def test6(n):
	...
	axes[0].set_ylabel('$n_{' + oiLabel + '}/n_H$', fontsize = 12, rotation = "horizontal")
	axes[1].plot(zs, DeHss[:, n])
	axes[1].set_ylabel('$\Delta$', fontsize = 12, rotation = "horizontal")
	axes[2].plot(zs, vss[:, n] / 1.0e3)
	axes[2].set_ylabel('$v / \mathrm{kms}^{-1}$', fontsize = 12, rotation = "horizontal")
	axes[3].semilogy(zs, Tss[:, n])
	axes[3].set_ylabel('$T/K$', fontsize = 12, rotation = "horizontal")
	axes[4].plot(zs, opticalDepths(n, False))
	axes[4].set_xlabel("$z$")
	axes[4].set_ylabel('$\\tau_{' + oiLabel + '}$', fontsize = 12, rotation = "horizontal")
	plt.subplots_adjust(hspace = 0)
	fig.align_ylabels()
	plt.show()
